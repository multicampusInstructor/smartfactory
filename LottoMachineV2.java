...
public class LottoMachineV2 {
	...
	public LottoMachineV2() { /*기본 생성자와 동일하나 명시적으로 나타내었음*/	}
	...
	public void startMachine() {
		...
		for(int i=0;i<balls.size()-1;i++) { //비교 위치의 값을 가르킴(해당 위치에 값을 결정)
			//LottoBall temp1 = balls.get(i);
			for(int j=i+1;j<balls.size();j++) { // 비교대상의 위치 값을 가르킴
				//LottoBall temp2 = balls.get(j);
				if(balls.get(i).getNumber() > balls.get(j).getNumber()) { //오름 차순 비교
					// 교환
					LottoBall temp = balls.get(i);
